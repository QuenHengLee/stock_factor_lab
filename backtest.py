...
def calc_weighted_positions(position, position_limit):
    ...
    position = (
        position.div(total_weight.where(total_weight != 0, np.nan), axis=0)
        .fillna(0)
...
def sim(
    ...
    # 初始化資產
    assets = pd.DataFrame(
        index=position.index, columns=["portfolio_value", "cost", "init", "remain"]
    )
    ...
    for day in position.index:
        ...
        if stock.loc[day]["signal"] == False:
            if day == stock.index[0]:
                ...
                assets.loc[day, "portfolio_value"] = (
                    stock.loc[day].drop(["signal"]) * shares_df.loc[day]
                ).sum()
                assets.loc[day, ["cost", "init", "remain"]] = assets.shift(1).loc[
        ...
        # 再平衡（rebalance）
            ...
            if first_trading:
                first_trading = False
                buy_amount = (
                    init_portfolio_value
                    * position.loc[day]
                    / (stock.loc[day].drop(["signal"]) * (1 + buy_extra_cost))
                )
                ...
                sell_money = (
                    (stock.loc[day].drop(["signal"]) * pd.Series(prev_values)).sum()
                    * (1 - sell_extra_cost)
                ) + assets.shift(1).loc[day, "remain"]

                # 計算每一支股票買多少張
                shares_df.loc[day] = np.floor(
                    sell_money
                    * position.loc[day]
                    / (stock.drop(["signal"], axis=1).loc[day] * (1 + buy_extra_cost))
                )

                sell_cost = (
                    stock.loc[day].drop(["signal"]) * pd.Series(prev_values)
                ).sum() * sell_extra_cost
                buy_cost = (
                    shares_df.loc[day]
                    * stock.loc[day].drop(["signal"])
                ...
                remain = (
                    ...
                    - (
                        shares_df.loc[day]
                        * stock.drop(["signal"], axis=1).loc[day]
                        * (1 + buy_extra_cost)
                    ).sum()
                )
            ...
            assets.loc[day, ["cost", "init", "remain"]] = [
                total_cost,
                sell_money,
                remain,
            ]
