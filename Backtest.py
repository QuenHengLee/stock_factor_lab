...
class Backtest:
    def __init__(
        ...
        self.assets = pd.DataFrame(
            ...
            columns=["portfolio_value", "cost", "init", "remain"],
        )
        self.assets["init"] = self.init_portfolio_value
        self.shares_df = pd.DataFrame(
            0, index=self.stock.index, columns=self.stock.columns.drop(["signal"])
        )
        self.prev_values = {}
    ...
    def calc_weighted_positions(self):  # 計算權重
        ...
        self.position = (
            self.position.div(
                self.total_weight.where(self.total_weight != 0, np.nan), axis=0
            )
            .fillna(0)
            .clip(-abs(self.position_limit), abs(self.position_limit))
        )
                    ...
                    if abs(weight_difference).sum() == 0:
                        ...
                        for s, w in weight_difference.items():
                            ...
                            if w < 0:
                                ...
                                sell_stock_shares_list[s] = np.floor(
                                    (abs(w) / self.position.shift(1).loc[day][s])
                                    * self.prev_values[s]
                                )
                            elif w > 0:
                        ...
                        for s, w in buy_stock_shares_list.items():
                            self.shares_df.loc[day, s] = np.floor(
                                buy_money_per_stock
                        ...
                        sell_cost = (
                            (self.shares_df.shift(1).loc[day] - self.shares_df.loc[day])
                            * self.stock.loc[day].drop(["signal"])
                            * self.sell_extra_cost
                        ).sum()
                        buy_cost = (
                            self.shares_df.loc[day]
                            * self.stock.loc[day].drop(["signal"])
